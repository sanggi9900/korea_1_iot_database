...
# : . 자기 조인(self), 상호 조인(across join)

###  1. 내부 조인 ###
# 두 테이블의 공통된 값을 기준으로 데이터를 결합하는 조인 방식
# : 테이블 간에 일치한느 행만 결과에 반환됨
# - 일치하지 않는 경우 출력되지 X
...
# cf) inner join의 경우 축약혀응로 join 키워드만 사용가능 
...
where
	M.member_age between 25 and 30 -- 특정 나이대 결정
group by
