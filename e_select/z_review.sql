...
select  name, grade, contact from `members`;
...
select name, join_date, grade from `member` 
-- 
where grade  in('Gold', 'Platinum', 'Diamond');

select name, join_date, grade from `member` 
where grade  not in('Gold', 'Platinum', 'Diamond'); -- not in : 포함되지 않는 경우\
...
select member_id,  max(purchase_date) as lase_purchase_date
from
	purchases
group by member_id  -- (member_id 끼리 묶는다)회원 별로 구매데이터 묶어
order by lase_purchase_date desc -- 그 데이터들 중에 제일 최근 데이터만을 뽑아
limit 3; -- 상위 3명만을 반환
