...
create table `student_course` (
    ...
    # >> 해당 각가의 컬럼이 컬럼 내에서는 중복 가능하지만 둘을 합친 값은 테이블 내에서 고유
    #	: student_i와 course_id가 함께 기본 키 역활을 함.
...
where
	professor_name = '원신희';
-- 오류 없이 수정이 이루어지지만 각 행에서 조건의 데이터를 찾아 일일이 변경을 해야되는 메모리 부담이 존재
...
where
	student_id = 3;
-- 학생에 대한 데이터가 존재 하지 않을 경우 '강의'에 대한 정보도 사라짐
...
values
	...
	(2, 101),
	(3, 101),
	(3, 103); -- 복합 키 설정이기 때문에 두 개의 컬럼을 합친 내용이 다른 레코드와 중복 xa
